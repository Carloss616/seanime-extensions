...
### Fixed
- Scanning a manga that isn't in your **reading** list from its entry page no longer adds it to the plugin's list. Only manga in your reading list belong in the update summary — a planned/paused/completed manga you scan for its per-source details now shows those details without leaking into the general list.

### Changed
- In-progress scan buttons (**Scan this manga**, the per-source rescan icon) now show a native spinner instead of an hourglass label, matching the rest of the UI.
