...
// fire in SEPARATE goja pool runtimes — they cannot close over anything
// declared at module scope or in init() (see seanime's internal/extension_repo/
// goja_plugin.go bindHooks + internal/plugin/ui/*.go: the callback is
// serialized via .toString() and recompiled in a fresh executor). Every
// outer callback body below is therefore fully self-contained — string
// constants are inlined at each call site, and Pre→Post payloads use
// `$store` (cross-runtime channel). Bare TypeScript interfaces are fine
// (they vanish after esbuild).
//
// Inner UI callbacks (event handlers registered via `ctx.eventHandler` /
// `ctx.registerEventHandler`, tray.render bodies, etc.) close over their
// enclosing $ui.register callback — anything declared inside that outer
// callback IS visible to them at runtime.
//
// `MUClient` lives in `lib/mu-client.ts` (a single source of truth). Both
// the post-hook IIFE and the $ui.register callback inline it via
// `// @inline ./lib/mu-client.ts` markers — build.py substitutes each
// marker with the file's contents. The class ends up declared inside each
// isolated runtime that needs it, but the TypeScript source has one
// definition. See [CLAUDE.md] for the marker mechanics.
...
function init() {
    ...
    const onPost = (event: { mediaId?: number; next(): void }) => {
        try {
            ...
            ;(async () => {
                ...
                // Inline `MUClient` (and friends) from `lib/mu-client.ts`. The
                // marker is replaced by build.py with the lib source — same
                // class definition the UI scope uses, but landing inside this
                // IIFE so it survives the goja worker's `.toString() + eval`
                // recompilation (the worker has no view of module scope).
                ...
                if (!externalId) {
                    console.warn(
                        "[mangaupdates-sync] no MU link for media " + mediaId +
    ...
    $ui.register((ctx) => {
        ...
        ctx.effect(() => {
            ...
            try { media = $anilist.getManga(id) } catch (_) { media = undefined }
        ...
        const resolveMULink = (mediaId: number): { url?: string; title?: string } => {
            let media: $app.AL_BaseManga | undefined
            try { media = $anilist.getManga(mediaId) } catch (_) { media = undefined }
            if (!media) return {}
            const customPrefix = "ext_custom_source_mangaupdates|END|"
            if (media.siteUrl && media.siteUrl.indexOf(customPrefix) === 0) {
                const url = media.siteUrl.substring(customPrefix.length)
                const title = (media.title && (media.title.english || media.title.userPreferred || media.title.romaji)) || undefined
                return { url, title }
            }
            const link = $storage.get<MULink>("mu_link_" + mediaId)
            if (!link) return {}
            const url = link.seriesUrl
            ...
            async (els) => {
                ...
                a.setInnerHTML(
                    '<button type="button" class="UI-Button_root whitespace-nowrap font-semibold rounded-lg inline-flex items-center transition ease-in text-center justify-center focus-visible:outline-none focus-visible:ring-2 ring-offset-1 ring-offset-[--background] focus-visible:ring-[--ring] disabled:opacity-50 disabled:pointer-events-none shadow-none text-[--gray] border border-transparent bg-transparent hover:underline active:text-gray-700 dark:text-gray-300 dark:active:text-gray-200 UI-IconButton_root p-0 flex-none text-xl h-8 w-8 px-0">' +
                        '<span class="md:inline-block">' + MU_ICON_SVG + '</span>' +
                    '</button>',
                )
        ...
        // Inline `MUClient` from lib/. The $ui.register callback itself runs
        // in an isolated goja runtime (same recompile model as the hooks), so
        // even though the lib is concatenated at module scope, this scope
        // can't see it — we have to inject the class here too.
        // @inline ./lib/mu-client.ts
        ...
        async function runSearch(query: string) {
            ...
            try {
                ...
                for (const r of arr) {
                    ...
                    if (!sid) continue
                    const rec = r.record
                    const year = rec.year ? parseInt(rec.year) : undefined
                    out.push({
                        id: String(sid),
                        title: rec.title || "(untitled)",
                        year: !isNaN(year as number) ? year : undefined,
                        cover: rec.image && rec.image.url
