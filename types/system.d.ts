...
declare namespace $filepath {
  ...
  function walk(
    root: string,
    walkFn: (path: string, info: $os.FileInfo, err: GoError) => GoError,
  ): void;
  ...
  function walkDir(
    root: string,
    walkDirFn: (path: string, d: $os.DirEntry, err: GoError) => GoError,
  ): void;
}
...
declare namespace $downloader {
  ...
  function watch(
    downloadId: string,
    callback: (progress: DownloadProgress | undefined) => void,
  ): () => void;
...
declare namespace $mime {
   ...
   */
  function parse(contentType: string): {
    mediaType: string;
