...
- `src/_components/` — declarative `$ui` builders: `entry-list`, `divider`, `tray-header` (a list-item modal header — left icon, title, subtitle, right-aligned nodes; `title`/`iconUrl` default to the owning extension's manifest `name`/`icon` via the `__MANIFEST_NAME__` / `__MANIFEST_ICON__` bundle-time literals — siblings of `__MANIFEST_ID__`, all injected by `scripts/build.ts`'s `define` and declared in [types/augment.d.ts](types/augment.d.ts)). (No `pill`/`alert-box` — use the native `tray.badge` / `tray.alert`. `tray.alert` only takes `title`/`description`/`intent` strings, so banners that need action buttons render the buttons as sibling `tray.flex` rows below the alert, not inside it.)

**Tray spacing convention:** vertical rhythm comes from the container `tray.stack` / `tray.flex` **`gap`** on a 4px grid, three tiers — `gap: 1` (4px) micro-pairs (title↔subtitle, value↔label), `gap: 2` (8px) inside a section, `gap: 4` (16px) between sections at the page level. Do NOT add `marginTop`/`marginBottom` to create spacing between siblings (that produced inconsistent 6/10/14px totals); `padding` is for box insets only, also snapped to 4/8/12. Each **section is a self-contained stack** (its own `gap: 2`), so it can be composed at any page-level gap without the outer gap leaking into its rows — `renderEntryListSection` returns one stack node, not a flat array (don't spread it). `divider` is a bare 1px line with no margin — the parent stack's gap spaces it. A section that is the first child under a `tray-header` passes `leadingDivider: false` so its rule doesn't double up with the header's bottom border. Tight pairs that must stay close inside a 16px page stack (an alert + its action box, a heading + its rows) get wrapped in their own `gap: 2` stack.
