...
### `undefined`-valued option keys crash `tray.*` bindings — omit, don't set to `undefined`

A tray component prop typed `className?: string` (or any optional string prop on `tray.badge` / `tray.button` / …) must be **absent** when you have no value — NOT present with value `undefined`. A JS object literal keeps `undefined`-valued keys (unlike JSON), so `tray.badge(label, { intent, className: undefined })` hands goja an object where `className` *exists*; the Go binding then type-asserts it as a string and panics: `TypeError: expected string, got nil at ...jsBadge-fm`, killing the whole tray render (`renderTrayScheduled` → `renderComponents`). A bare `tray.badge(label, { intent })` works precisely because the key is absent, not present-but-`undefined`. Caught in `_components/entry-list.ts`: it forwarded `style: row.status.style, className: row.status.className` unconditionally, so any row whose `status`/`warn` set only `{label, intent}` (e.g. `manga-source-updates`'s per-source detail rows) crashed on open. Fix is to spread the key in only when set: `...(row.status.className ? { className: row.status.className } : {})`. Applies to every optional string/scalar prop crossing into a Go-bound `tray.*` builder — build the props object so unset keys never appear.
...
- **Hooking seanime's OWN buttons/modals/menus (not injecting new UI).** To make a native control also trigger plugin logic, attach a listener rather than decorate. seanime's confirm modals (`[role="dialog"]`) and dropdown menus (`[role="menu"]`) are **radix portals at the document root** — they don't live under any page container, so observe them with their OWN root-level `dm.observe("[role='dialog']" / "[role='menu']", …)` (not scoped to a page node), and they re-mount on every open. The controls carry no `data-*` of their own, so: **gate by visible text** — the modal's `.UI-Modal__title` (lowercased) or the menu's `[role='menuitem']` `getText()` — to pick the specific one and skip siblings/other dialogs; and guard against double-attach with an attribute stamp on the element (`data-msu-reload-hooked` etc.), NOT handle bookkeeping — a re-mounted element loses the attr and is re-hooked, a reused one keeps attr + listener. A button that opens seanime's own confirm modal should be hooked at the modal's confirm button (fires on confirmation); one that fires immediately should either kick off the action directly (a single-entry scan) or open the plugin's own confirm (a heavy whole-list scan). seanime's UI shifts between versions here — the entry page's source refresh was a "Reload sources" **confirm modal** (hooked via `hookReloadModal` on `[role=dialog]`) but newer builds make it a **"Refresh source" button directly in `[data-chapter-list-header-container]`** with no modal (no `aria-haspopup`), hooked via `hookRefreshSourceButton` (match by button text, stamp `data-msu-refresh-source-hooked`, fire `probeMangaDetail` immediately); the library "Refresh sources" **menu item** (`hookRefreshMenu`) opens the plugin's own confirm. All three in `manga-source-updates` are the reference (keep the modal hook too — it still fires on older seanime). For a plugin's OWN confirm dialog, prefer a state-driven view-swap in `tray.render` over a controlled `tray.modal` (the native controlled-open modal didn't reliably close).
