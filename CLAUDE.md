...
- **`el.queryOne` / `el.appendChild` reject silently in denshi** (the desktop runtime) — the async call never resolves/rejects visibly, so the decorate fn dies right after with no error. Use `el.query(sel)` and take `[0]`, and `el.append(child)` (the pattern that works for the chapter-list bar), NOT `queryOne`/`appendChild`. Wrap the inject block in try/catch so a failure logs instead of vanishing.
