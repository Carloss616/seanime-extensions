...
- `src/_utils/` — `logger`, `anilist-status`, `custom-source-id` (the mediaId encode/decode helpers), `dom-read` (`domAttr` / `parseDomNumber` — denshi-safe attribute + text reads), plus `mangaupdates/` and `local-catalog/` domain helpers. Each extension's per-extension `utils/*` wrapper (e.g. `mu-client.ts`) now extends a shared base (`_utils/mangaupdates/client.ts`) instead of reimplementing it.
